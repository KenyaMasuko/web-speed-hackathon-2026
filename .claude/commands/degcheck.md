...
- VRT・スコア計測など長時間コマンドはバックグラウンド (`run_in_background`) で実行し、待ち時間に並列で次の準備を進めてよい
- バックグラウンドタスク完了通知を受けたら `TaskOutput block=false` で出力を確認し、**即座に結果を報告する**
- `TaskOutput block=true` で再度待ちに入らないこと（ハングの原因になる）
